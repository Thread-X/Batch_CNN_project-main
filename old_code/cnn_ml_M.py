...
def copy_data_at_D1(copy_times,raw_data):
    tem = raw_data
    for i in range(copy_times-1):
        tem = np.concatenate((raw_data, tem), axis=1)
    return tem
...
# targets_to_one_hot = torch.nn.functional.one_hot(5)

# train_data = copy_data_at_D1(COPY_TIMES,train_data)
...
# test_data = copy_data_at_D1(COPY_TIMES,test_data)
